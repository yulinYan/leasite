<template>
	<div class="index">
		...
		<div class="main-block" >
			<div class="block-item" v-for="app in appList" @click="openDialog(`${app.name}`)">
		...
		<div class="status-bar">
			<!--状态栏-->
		<SystemMenu v-if="systemMenuVisible"></SystemMenu>
		...
		<DialogUrl  v-for="(app,index) in activeApps" v-if="app.isShowDialog" width="100%" @callBackFun="closeParentDialog" :appObj=app :key="`${app.name}`" :ref="'dialogUrl'+index"></DialogUrl>
			...
			openDialog(appName){
				...
				if(this.activeApps.length > 0){
					for(let i=0;i<this.activeApps.length;i++){
						if(this.activeApps[i].name == appName){
							searchIndex = i;
							break;
						}
					}	
				...
				if(searchIndex ==-1){
					if(this.activeApps.length >= 20){
			            this.$message({
			              type: 'info',
			              message: '最多只能同是打开20个应用'
			            });
			            return;
					}
					let chioceApp = {};
					for(let i=0;i<this.appList.length;i++){
						if(this.appList[i].name == appName){
